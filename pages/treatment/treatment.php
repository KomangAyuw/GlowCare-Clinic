...
<body
    ...
    <header
        ...
        <div
            ...
            <!-- Brand Logo -->
            <a class="font-headline-lg text-headline-lg text-primary inline-flex items-center"
                href="../../index.php"><span class="material-symbols-outlined text-primary text-3xl mr-2 align-middle"
    ...
    <main class="pt-[80px]">
        ...
        <section class="relative w-full h-[50vh] min-h-[400px] flex items-center justify-center overflow-hidden">
            ...
            <div class="relative z-10 text-center max-w-3xl px-margin-desktop mx-auto">
                <span
                    class="inline-block px-4 py-1.5 mb-md backdrop-blur-md font-label-sm text-label-sm rounded-full tracking-widest uppercase bg-primary/10 text-primary border border-primary/25 shadow-sm">Clinical
                    Excellence</span>
                <h1
                    class="font-display-lg text-display-lg md:text-[42px] text-on-background mb-md font-serif leading-tight">
                    Our Clinical Treatments</h1>
                <p class="font-body-lg text-body-lg text-on-surface-variant max-w-2xl mx-auto leading-relaxed">
                    Experience the pinnacle of medical-grade aesthetics. Our treatments combine advanced clinical
                    technology with a serene luxury experience, curated by expert practitioners for your skin's health.
                </p>
            </div>
        </section>
        ...
        <section
            class="sticky top-[72px] z-40 bg-surface/90 backdrop-blur-md border-b border-outline-variant/30 py-4 shadow-sm">
            <div
                class="max-w-[1200px] mx-auto px-margin-mobile md:px-margin-desktop flex flex-col md:flex-row md:items-center justify-between gap-md">
                <!-- Category Filter Tabs -->
                <div class="flex items-center gap-xs overflow-x-auto whitespace-nowrap no-scrollbar pb-2 md:pb-0"
                    id="filter-container">
                    <button data-target="all"
                        class="filter-tab active font-label-md text-label-md px-5 py-2.5 rounded-full transition-all duration-300">All
                        Treatments</button>
                    <button data-target="acne"
                        class="filter-tab text-on-surface-variant hover:text-primary font-label-md text-label-md px-5 py-2.5 rounded-full transition-all duration-300">Acne
                        &amp; Clear Skin</button>
                    <button data-target="anti-aging"
                        class="filter-tab text-on-surface-variant hover:text-primary font-label-md text-label-md px-5 py-2.5 rounded-full transition-all duration-300">Anti-Aging</button>
                    <button data-target="brightening"
                        class="filter-tab text-on-surface-variant hover:text-primary font-label-md text-label-md px-5 py-2.5 rounded-full transition-all duration-300">Brightening</button>
                    <button data-target="body"
                        class="filter-tab text-on-surface-variant hover:text-primary font-label-md text-label-md px-5 py-2.5 rounded-full transition-all duration-300">Hair
                        &amp; Body</button>
                </div>
                <!-- Premium Search Input -->
                <div class="relative w-full md:w-80">
                    <span
                        class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-variant/50 pointer-events-none text-xl">search</span>
                    <input type="text" id="search-input" placeholder="Search treatments..."
                        class="w-full pl-10 pr-10 py-2.5 bg-surface-container-low border border-outline-variant/50 rounded-full text-body-md text-on-surface placeholder:text-on-surface-variant/40 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/30 transition-all duration-300 shadow-inner">
                    <button id="clear-search"
                        class="absolute right-3 top-1/2 -translate-y-1/2 text-on-surface-variant/40 hover:text-primary hidden transition-colors duration-200">
...
?>
                <div data-category="<?= $cat_class ?>" data-name="<?= strtolower($nama) ?>"
                    ...
                    <div class="relative h-[240px] w-full overflow-hidden">
                        <img alt="<?= $nama ?>"
                            ...
                            src="<?= $img ?>">
                        <span
                            class="absolute top-4 left-4 bg-surface/90 backdrop-blur-md text-primary font-label-sm text-label-sm px-3.5 py-1.5 rounded-full border border-primary/20 shadow-sm font-semibold"><?= $kat ?></span>
                    </div>
                    <div class="p-md flex flex-col flex-grow">
                        <div class="flex justify-between items-start mb-3">
                            <h3
                                class="font-headline-md text-headline-md text-on-background leading-tight font-serif font-semibold group-hover:text-primary transition-colors duration-300">
                                <?= $nama ?></h3>
                            <div
                                class="flex items-center text-on-surface-variant/70 shrink-0 bg-surface-container-low px-2.5 py-1 rounded-md ml-2 border border-outline-variant/20">
                                <span class="material-symbols-outlined text-sm mr-1"
                                    style="font-size: 16px;">schedule</span>
                                <span class="font-label-sm text-label-sm font-medium"><?= $durasi ?></span>
                            </div>
                        </div>
                        <p class="font-body-sm text-body-sm text-on-surface-variant/90 mb-lg flex-grow leading-relaxed">
                            <?= $desc ?></p>
                        <a href="detail_treatment.php?id=<?= $tr['id'] ?>"
                            class="w-full py-3 bg-surface-container-high text-primary font-label-md text-label-md rounded-xl border border-primary/25 hover:bg-primary hover:text-on-primary hover:border-transparent transition-all duration-300 uppercase tracking-widest text-center block inline-flex items-center justify-center gap-2 font-semibold shadow-sm hover:shadow-md"><span
                                class="material-symbols-outlined" style="font-size:18px;">visibility</span> Lihat
                            Detail</a>
                    </div>
                </div>
                <?php endforeach; ?>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-lg items-start">
                ...
                <div class="flex flex-col gap-md">
                    ...
                    <p class="font-body-lg text-body-lg text-on-surface-variant leading-relaxed">
                        Setiap perawatan kami didukung oleh teknologi medis tercanggih yang telah tersertifikasi
                        internasional, memastikan hasil maksimal dengan tingkat keamanan tertinggi bagi pasien kami.
                    </p>
                    <div class="grid grid-cols-2 gap-md mt-lg">
                        ...
                        <div class="flex items-center gap-sm">
                            <div class="w-10 h-10 rounded-full bg-secondary-container flex items-center justify-center">
                                <span class="material-symbols-outlined text-secondary"
                                    style="font-variation-settings: &quot;FILL&quot; 1;">clinical_notes</span>
                            </div>
                ...
                <div class="relative rounded-2xl overflow-hidden shadow-lg">
                    ...
                    <div
                        class="absolute bottom-6 left-6 right-6 bg-surface/90 backdrop-blur-md p-md rounded-xl border border-outline-variant/30">
        ...
        <section class="py-xl px-margin-mobile md:px-margin-desktop max-w-[1000px] mx-auto text-center mb-xl">
            <div
                class="bg-surface-container-low p-xl rounded-2xl relative overflow-hidden border border-primary-container/30 shadow-sm flex flex-col items-center text-center">
                ...
                <!-- Refined Body Text -->
                <p
                    class="font-body-lg text-body-lg text-on-surface-variant max-w-xl mx-auto mb-lg relative z-10 leading-relaxed">
                    Schedule a comprehensive consultation with our clinical experts to develop a personalized treatment
                    plan tailored to your unique goals.
                </p>
                <!-- High-Contrast Primary Button -->
                <a href="<?= isset($_SESSION['user_id']) ? '../user/dashboarduser.php?page=daftar-konsul' : '../auth/Signin.php' ?>"
                    class="inline-block relative z-10 bg-primary text-on-primary font-label-md text-label-md px-xl py-4 rounded-lg hover:-translate-y-1 hover:shadow-lg transition-all duration-300 uppercase tracking-wider">
                    Booking Now
                </a>
                <!-- Subtle Bottom Accent -->
                <div
                    class="absolute bottom-0 left-1/2 -translate-x-1/2 w-32 h-1 bg-primary-container/40 rounded-t-full">
                </div>
            </div>
        </section>
    </main>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        function filterCards() {
            treatmentCards.forEach(card => {
                const matchesCategory = (activeCategory === 'all' || card.dataset.category ===
                    activeCategory);
                const cardName = card.dataset.name || '';
                const cardDesc = card.dataset.desc || '';
                const matchesSearch = !searchQuery || cardName.includes(searchQuery) || cardDesc
        ...
        filterTabs.forEach(tab => {
            tab.addEventListener('click', (e) => {
                ...
                const readableParam = Object.keys(paramMap).find(key => paramMap[key] ===
                    activeCategory);
