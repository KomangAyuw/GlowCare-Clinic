...
<body style="display:block; background:#fbf9f4;">
    ...
    <div class="topbar" style="position:static; margin-bottom: 0;">
        <div style="display:flex; align-items:center; gap:16px;">
            <a href="dashboardDokter.php"
                style="color:#735a39; text-decoration:none; font-size:18px; font-weight:bold; background:#f5f3ee; padding:6px 12px; border-radius:8px;"
                title="Kembali ke Dashboard">←</a>
        </div>
        <div></div>
    </div>
    ...
    <div class="chat-container">
        ...
        <div class="chat-sidebar">
            ...
            <div class="chat-session-item <?= ($selected_pasien_id > 0 && $c['pasien_id'] == $selected_pasien_id) ? 'active' : '' ?>"
                onclick="window.location.href='chat.php?appt_id=<?= $c['appointment_id'] ?>'">
                <div style="font-weight:500; font-size:14px; color:#2D3436"><?= htmlspecialchars($c['nama_pasien']) ?>
                </div>
                <div style="font-size:12px; color:#735a39; margin-top:4px">
                    <?= htmlspecialchars($c['nama_treatment'] ?? 'Konsultasi') ?></div>
                <div style="font-size:11px; color:#64748b; margin-top:6px">Terakhir:
                    <?= date('d M Y, H:i', strtotime($c['tanggal'] . ' ' . $c['jam'])) ?></div>
            </div>
            <?php endforeach; ?>
            <?php endif; ?>
        </div>
        ...
        <div class="chat-main">
            ...
            <div
                style="flex:1; display:flex; align-items:center; justify-content:center; flex-direction:column; color:#7a7571;">
            ...
            <div class="chat-header"
                style="flex-wrap: wrap; gap: 12px; padding: 12px 24px; display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid #d1c4b8;">
                <div class="chat-pasien-info" style="flex: 1; min-width: 200px;">
                    <div class="avatar" style="width:40px; height:40px; font-size:16px; margin-right:0;">
                        <?= substr(htmlspecialchars($pasien['nama']), 0, 1) ?></div>
                    <div>
                        <div
                            style="font-family:'Playfair Display', serif; font-size:18px; color:#2D3436; font-weight: 500;">
                            <?= htmlspecialchars($pasien['nama']) ?></div>
                        <div style="font-size:12px; color:#735a39; margin-top: 2px;">
                            <?= htmlspecialchars($pasien['usia'] ?? '-') ?> Tahun
                            <?= !empty($pasien['jenis_kelamin']) ? '· ' . htmlspecialchars($pasien['jenis_kelamin']) : '' ?>
                        </div>
                        <?php if (count($session_history) > 1): ?>
                        <div
                            style="margin-top: 6px; display: flex; align-items: center; gap: 8px; font-size: 11px; color: #64748b;">
                            <span style="font-weight: 500;">Pilih Sesi Janji Temu:</span>
                            <select onchange="window.location.href='chat.php?appt_id=' + this.value"
                                style="padding: 2px 6px; border-radius: 4px; border: 1px solid #d1c4b8; font-family: 'DM Sans', sans-serif; font-size: 11px; background: #faf8f5; color: #2d3436; outline: none; cursor: pointer;">
                                ...
                                <option value="<?= $history['appt_id'] ?>" <?= $selectedStr ?>>
                                    <?= $dateStr ?> (<?= htmlspecialchars($history['nama_treatment'] ?? 'Konsultasi') ?>
                                    - <?= $statusStr ?>)
                                </option>
                                <?php endforeach; ?>
                            </select>
                ...
                <div style="display:flex; flex-direction:column; align-items:flex-end; gap:4px; text-align: right;">
                    ...
                    <span
                        style="font-size:11px; color:#735a39; font-weight: 500;"><?= htmlspecialchars($selected_appt_info['nama_treatment'] ?? 'Konsultasi') ?></span>
                    <span
                        style="font-size:10px; color:#9a8f87;"><?= date('d M Y, H:i', strtotime($selected_appt_info['tanggal'] . ' ' . $selected_appt_info['jam'])) ?>
                        WIB</span>
                </div>
                <?php endif; ?>
            </div>
            ...
            <div class="chat-messages" id="chatMessages">
                <div style="text-align:center; font-size:11px; color:#7f8c8d; margin:10px 0;">Sesi Konsultasi Dimulai
                </div>
            </div>

            <div class="chat-input-area">
                <input type="file" id="imageUpload" accept="image/*" style="display:none" onchange="sendImage(this)">
                <button type="button" class="chat-btn" style="background: #eae8e3; color: #735a39;"
                    title="Upload Gambar" onclick="document.getElementById('imageUpload').click()">
                    📷
                </button>
                <textarea class="chat-input" placeholder="Ketik pesan Anda di sini..." rows="1"
    ...
    function renderMessages(messages) {
        ...
        if (!box) return;
        box.innerHTML =
            '<div style="text-align:center;font-size:11px;color:#7f8c8d;background:#f0ede8;padding:4px 12px;border-radius:20px;margin:4px auto;display:inline-block;">Sesi Konsultasi Dimulai</div>';

        messages.forEach(msg => {
            const isSent = msg.sender_type === 'Dokter';

            const group = document.createElement('div');
            group.style.cssText = 'display:flex;flex-direction:column;align-items:' + (isSent ? 'flex-end' :
            ...
            if (msg.image_url) {
                content +=
                    `<img src="../../${msg.image_url}" style="max-width:100%;border-radius:10px;margin-bottom:4px;display:block;" alt="Gambar">`;
            }
