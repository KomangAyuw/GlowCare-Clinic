...
<body>
        ...
        <div class="topbar" style="justify-content: flex-end;">
            ...
            <div class="topbar-right">
                <a href="#" onclick="showLogoutModal(); return false;" class="logout-btn"
                    style="width: auto !important; padding: 6px 16px;">Logout</a>
            </div>
        </div>
            ...
            <div class="panel active" id="panel-dashboard">
                ...
                <div class="hero-banner" style="margin: -36px -40px 36px -40px;">
                    <div class="hero-greeting">
                        <?= ['Minggu','Senin','Selasa','Rabu','Kamis','Jumat','Sabtu'][date('w')] ?>,
                        <?= date('d M Y') ?></div>
                    <div class="hero-name">Halo, <em><?= htmlspecialchars($admin['username']) ?></em></div>
                    <div class="hero-sub">Selamat datang kembali di GlowCare Clinic. Yuk, kelola operasional klinik
                        dengan baik.</div>
                </div>
                <div class="stats-row">
                    <div class="stat-card">
                ...
                <div class="two-col">
                    <div class="card">
                        <div class="card-header">
                            <div class="card-title">Kunjungan Pasien — <?= date('Y') ?></div>
                        </div>
                            ...
                            <div class="chart-bars">
                                ...
                                <div class="chart-bar-wrap">
                                    <div class="chart-bar" style="height:<?=$h?>%"
                                        title="<?=$chart_data[$m]?> kunjungan"></div>
                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <a class="card-action"
                                onclick="showPanel('aktivitas',document.querySelector('[onclick*=aktivitas]'))">Semua
                                →</a>
                        </div>
                        <div class="activity-list">
                            ...
                            <div class="activity-item">
                                ...
                                <div>
                                    <div class="activity-text"><strong><?= htmlspecialchars($ak['judul']) ?></strong> —
                                        <?= htmlspecialchars($ak['deskripsi']) ?></div>
                                    <div class="activity-time"><?= date('d M Y, H:i',strtotime($ak['created_at'])) ?>
                                    </div>
                                </div>
                            </div>
                            <?php endwhile; ?>
            ...
            <div class="panel" id="panel-pasien">
                <div class="page-header">
                    <div>
                        ...
                        <p class="section-sub">Kelola seluruh data pasien terdaftar</p>
                    </div>
                ...
                <div class="card">
                        ...
                        $usia=$p['tanggal_lahir']?(date('Y')-date('Y',strtotime($p['tanggal_lahir']))).''  :'-'; ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <div style="font-size:11px;color:#7a7571"><?= htmlspecialchars($p['email']??'-') ?>
                                    </div>
                                </td>
                                <td><?= htmlspecialchars($p['treatment_terakhir']??'-') ?></td>
                                <td style="text-align:center"><?= $p['total_kunjungan'] ?></td>
                                <td><?= $p['status']==='Aktif'?'<span class="badge badge-green">Aktif</span>':'<span class="badge badge-gray">Tidak Aktif</span>' ?>
                                </td>
                                <td>
                                    <button class="act-btn"
                                        style="border: 1px solid #d1c4b8; padding: 4px 10px; margin-right: 4px;"
                                        onclick="editPasien(<?= htmlspecialchars(json_encode($p)) ?>)"
                                        title="Edit">Edit</button>
                                    <button class="act-btn"
                                        style="border: 1px solid #e05050; color: #e05050; padding: 4px 10px;"
                                        onclick="confirmDelete('hapus_pasien.php','<?= $p['id'] ?>','pasien <?= htmlspecialchars(addslashes($p['nama'])) ?>')"
                    ...
                    <div style="padding:14px 18px;border-top:1px solid #F9F7F2;font-size:12px;color:#7a7571">Total
                        <?= number_format($total_pasien) ?> pasien terdaftar</div>
                </div>
            </div>
            ...
            <div class="panel" id="panel-dokter">
                <div class="page-header">
                    <div>
                        ...
                        <p class="section-sub">Kelola profil dan spesialisasi dokter</p>
                    </div>
                    <button class="btn-add"
                        onclick="document.getElementById('form-dokter').reset(); document.getElementById('md-id').value=''; document.getElementById('md-current-foto').value=''; document.getElementById('md-foto').required=true; openModal('modal-dokter')">+
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    $foto = !empty($d['foto']) 
                                        ...
                                        : '';
                                     if ($foto): ?>
                                        <img class="avatar" src="<?= htmlspecialchars($foto) ?>"
                                            style="object-fit:cover; width:30px; height:30px; border-radius:50%; margin-right:8px;"
                                ...
                                <td><span class="badge badge-pink"><?= htmlspecialchars($d['spesialisasi']) ?></span>
                                </td>
                                ...
                                <td>
                                    ...
                                    <button class="act-btn"
                                        style="border: 1px solid #e05050; color: #e05050; padding: 4px 10px;"
                                        onclick="confirmDelete('hapus_dokter.php','<?= $d['id'] ?>','dokter <?= htmlspecialchars(addslashes($d['nama'])) ?>')"
                                        title="Hapus">Hapus</button>
                                </td>
                            </tr>
                            <?php endwhile; ?>
                ...
                ?>
                    <div class="jadwal-doctor-card">
                        ...
                        $j_foto = !empty($grp['foto']) 
                            ...
                            : '';
                        if ($j_foto): ?>
                            <img class="jdc-avatar" src="<?= htmlspecialchars($j_foto) ?>"
                                style="object-fit:cover; width:44px; height:44px; border-radius:50%; margin-right:16px;"
                                alt="<?= htmlspecialchars($grp['nama']) ?>">
                            <?php else: ?>
                        ...
                        <div class="jdc-body">
                            ...
                            <div class="jdc-row">
                                ...
                                <span
                                    class="jdc-treatment"><?= htmlspecialchars($jj['nama_treatment'] ?? 'Semua Treatment') ?></span>
                                <span
                                    style="text-align:center;font-size:13px;color:#2D3436;font-weight:500"><?= $jj['max_pasien'] ?></span>
                                <span style="text-align:center">
                                    <?php $st = $jj['status'] ?? 'Aktif'; ?>
                                    <span
                                        class="badge <?= $st === 'Aktif' ? 'badge-green' : 'badge-gray' ?>"><?= htmlspecialchars($st) ?></span>
                                </span>
                                <span class="jdc-actions">
                                    <button class="act-btn jdc-btn-edit"
                                        onclick="editJadwal(<?= htmlspecialchars(json_encode($jj)) ?>)"
                                        title="Edit">Edit</button>
                                    <button class="act-btn jdc-btn-hapus"
                                        onclick="confirmDelete('hapus_jadwal.php','<?= $jj['id'] ?>','jadwal ini')"
                                        title="Hapus">Hapus</button>
                                </span>
                            </div>
                            <?php endforeach; ?>
            ...
            <div class="panel" id="panel-treatment">
                <div class="page-header">
                    <div>
                        ...
                        <p class="section-sub">Data treatment yang tampil di halaman utama</p>
                    </div>
                        ...
                        if ($img && strpos($img, 'http') !== 0) {
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <?php else: ?>
                                    <div
                                        style="width:50px;height:38px;background:#d1c4b8;border-radius:6px;display:flex;align-items:center;justify-content:center">
                                        💉</div>
                                    <?php endif; ?>
                                </td>
                                <td>
                                    <div class="td-name"><?= htmlspecialchars($tr['nama']) ?></div>
                                </td>
                                <td><span class="badge badge-pink"><?= htmlspecialchars($tr['kategori']) ?></span></td>
                                <td style="font-size:13px;font-weight:500"><?= htmlspecialchars($tr['durasi']??'') ?>
                                </td>
                                <td style="max-width:200px;font-size:12px;color:#585552">
                                    <?= htmlspecialchars(mb_substr($tr['deskripsi_panjang']??$tr['deskripsi']??'',0,70)) ?>...
                                </td>
                                <td><?= $tr['status']==='Aktif'?'<span class="badge badge-green">Aktif</span>':'<span class="badge badge-gray">Nonaktif</span>' ?>
                                </td>
                                <td style="white-space:nowrap">
                                    <a class="act-btn"
                                        style="border: 1px solid #386663; color: #386663; padding: 4px 10px; margin-right: 4px; text-decoration:none; display:inline-block;"
                                        href="../treatment/detail_treatment.php?id=<?= $tr['id'] ?>" target="_blank"
                                        title="Preview">Preview</a>
                                    <button class="act-btn"
                                        style="border: 1px solid #d1c4b8; padding: 4px 10px; margin-right: 4px;"
                                        onclick="editTreatment(<?= htmlspecialchars(json_encode($tr)) ?>)"
                                        title="Edit">Edit</button>
                                    <button class="act-btn"
                                        style="border: 1px solid #e05050; color: #e05050; padding: 4px 10px;"
                                        onclick="confirmDelete('hapus_treatment.php','<?= $tr['id'] ?>','treatment <?= htmlspecialchars(addslashes($tr['nama'])) ?>')"
                                        title="Hapus">Hapus</button>
                                </td>
                            </tr>
                            <?php endwhile; ?>
                        ...
                        <div class="activity-item">
                            ...
                            <div style="flex:1">
                                <div class="activity-text">
                                    <strong><?= htmlspecialchars($lg['judul']) ?></strong><?= $lg['deskripsi']?' — '.htmlspecialchars($lg['deskripsi']):'' ?>
                                </div>
                                ...
                                <td>
                                    <div style="display:flex;align-items:center;gap:8px">
                                        ...
                                        <div>
                                            ...
                                            <?php if ($belum): ?>
                                            <span
                                                style="font-size:9px;letter-spacing:1px;text-transform:uppercase;color:#735a39">●
                                                Baru</span>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                ...
                                <td style="white-space:nowrap">
                                    ...
                                    <form method="POST" action="../../backend/admin/kelola_pesan.php"
                                        ...
                                        <button type="submit" class="act-btn"
                                            style="border: 1px solid #3dab74; color: #3dab74; padding: 4px 10px;"
                                            title="Tandai sudah dibaca">Sudah Dibaca</button>
                                    </form>
                                    <?php endif; ?>
            ...
            <div class="panel" id="panel-pengumuman">
                <div class="page-header">
                    <div>
                        ...
                        <p class="section-sub">Buat pengumuman baru yang akan dikirim sebagai notifikasi ke pasien &amp;
                            dokter</p>
                    </div>
                    ...
                    <table class="data-table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td style="max-width:300px; font-size:12px; color:#585552; line-height:1.5;">
                                    <?= nl2br(htmlspecialchars($p['konten'])) ?></td>
                                <td style="font-size:12px; color:#7a7571; white-space:nowrap;">
                                    <?= date('d M Y, H:i', strtotime($p['created_at'])) ?> WIB</td>
                                <td>
                                    <button class="act-btn"
                                        style="border: 1px solid #d1c4b8; padding: 4px 10px; margin-right: 4px;"
                                    ...
                                    <button class="act-btn"
                                        style="border: 1px solid #e05050; color: #e05050; padding: 4px 10px;"
                                        onclick="confirmDelete('hapus_pengumuman.php', '<?= $p['id'] ?>', 'pengumuman ini')"
                                        title="Hapus">Hapus</button>
                                </td>
                            </tr>
                            <?php endwhile; endif; ?>
            ...
            <div class="panel" id="panel-laporan">
                <div class="page-header">
                    ...
                    <div style="display:flex;gap:10px">
                        <a href="../../backend/admin/laporan_download.php?format=csv&lap_bulan=<?= $lap_bulan ?>&lap_tahun=<?= $lap_tahun ?>"
                            class="btn-lap-download btn-lap-csv">Download CSV</a>
                        <a href="../../backend/admin/laporan_download.php?format=print&lap_bulan=<?= $lap_bulan ?>&lap_tahun=<?= $lap_tahun ?>"
                            target="_blank" class="btn-lap-download btn-lap-print">Cetak PDF</a>
                    </div>
                </div>
                ...
                <div class="card">
                    ...
                    <table class="data-table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td colspan="7" style="text-align:center;color:#64748b;padding:40px">Tidak ada data
                            ...
                            <tr>
                                ...
                                <td><span
                                        class="badge badge-pink"><?= htmlspecialchars($lp['spesialisasi'] ?? '-') ?></span>
                                </td>
                                ...
                                <td style="text-align:center">
                                    <?php if ($lp['batal'] > 0): ?>
                                    <span class="badge"
                                        style="background:#fef0f0;color:#e05050"><?= $lp['batal'] ?></span>
                                    <?php else: ?>
                                    <span style="color:#b0bec5">—</span>
                                    <?php endif; ?>
                                </td>
                                <td style="text-align:right;font-weight:600;color:#3dab74">
                                    <?= rupiah((float)$lp['pendapatan']) ?></td>
                        ...
                        <tfoot>
                            <tr class="lap-tfoot-row">
                                ...
                                <td style="text-align:right;color:#3dab74"><strong><?= rupiah((float)$tot_p) ?></strong>
                                </td>
                                <td></td>
                            </tr>
                        </tfoot>
                    ...
                    <form method="GET" class="keu-filter-form">
                        ...
                        <div class="keu-filter-group">
                            ...
                            <select class="form-select" name="keu_jenis" onchange="this.form.submit()">
                                ...
                                <option value="Pemasukan" <?=$keu_filter_jenis==='Pemasukan'?'selected':''?>>Pemasukan
                                </option>
                                <option value="Pengeluaran" <?=$keu_filter_jenis==='Pengeluaran'?'selected':''?>>
                                    Pengeluaran</option>
                            </select>
                        </div>
                    </form>
            ...
            <div class="panel" id="panel-profil">
                <div class="page-header">
                    <div>
                        ...
                        <p class="section-sub">Kelola informasi akun admin</p>
                    </div>
                </div>
                <div class="profil-hero">
                    ...
                    <div class="profil-info">
                        ...
                        <div class="profil-meta">
                            <div>
                                <div class="profil-meta-label">Email</div>
                                <div class="profil-meta-value"><?= htmlspecialchars($admin['email']) ?></div>
                            </div>
                            <div>
                                <div class="profil-meta-label">Bergabung</div>
                                <div class="profil-meta-value"><?= date('d M Y',strtotime($admin['created_at'])) ?>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card">
                    <div class="card-header">
                        <div class="card-title">Ganti <em>Password</em></div>
                    </div>
                    <div style="padding:22px">
                        <form method="POST" action="../../backend/admin/ganti_password.php">
                            <div class="form-row">
                                <div class="form-group full"><label class="form-label">Password Saat Ini</label><input
                                        class="form-input" type="password" name="password_lama" placeholder="••••••••"
                                        required></div>
                                <div class="form-group"><label class="form-label">Password Baru</label><input
                                        class="form-input" type="password" name="password_baru"
                                        placeholder="Min. 8 karakter" required></div>
                                <div class="form-group"><label class="form-label">Konfirmasi</label><input
                                        class="form-input" type="password" name="konfirmasi"
            ...
            <form method="POST" id="form-pasien">
                ...
                <div class="form-row">
                    <div class="form-group"><label class="form-label">Nama Lengkap</label><input class="form-input"
                            name="nama" id="mp-nama" placeholder="Nama lengkap" required></div>
                    <div class="form-group"><label class="form-label">No. Pasien</label><input class="form-input"
                            name="no_pasien" id="mp-no" placeholder="P-XXXX" required></div>
                    <div class="form-group"><label class="form-label">Tanggal Lahir</label><input class="form-input"
                            type="date" name="tanggal_lahir" id="mp-tgl"></div>
                    <div class="form-group"><label class="form-label">Jenis Kelamin</label>
                        <select class="form-select" name="jenis_kelamin" id="mp-jk">
                            <option>Perempuan</option>
                            <option>Laki-laki</option>
                        </select>
                    </div>
                    <div class="form-group"><label class="form-label">No. Telepon</label><input class="form-input"
                            name="telepon" id="mp-telp" placeholder="+62"></div>
                    <div class="form-group"><label class="form-label">Email</label><input class="form-input"
                            type="email" name="email" id="mp-email" placeholder="email@contoh.com"></div>
                    <div class="form-group full"><label class="form-label">Alamat</label><input class="form-input"
                            name="alamat" id="mp-alamat" placeholder="Alamat lengkap"></div>
                    <div class="form-group"><label class="form-label">Status</label>
                        <select class="form-select" name="status" id="mp-status">
                            <option>Aktif</option>
                            <option>Tidak Aktif</option>
                        </select>
                    </div>
                    <div class="form-group full"><label class="form-label">Catatan Medis</label><textarea
                            class="form-textarea" name="catatan_medis" id="mp-catatan"
                            placeholder="Alergi, kondisi khusus..."></textarea></div>
                </div>
    ...
    <div class="modal-overlay" id="modal-dokter" onclick="closeModalOutside(event,'modal-dokter')">
        <div class="modal">
            ...
            <form method="POST" id="form-dokter" enctype="multipart/form-data">
                ...
                <div class="form-row">
                    <div class="form-group"><label class="form-label">Nama Lengkap</label><input class="form-input"
                            name="nama" id="md-nama" placeholder="Dr. Nama Lengkap" required></div>
                    <div class="form-group"><label class="form-label">No. STR / SIP</label><input class="form-input"
                            name="no_str" id="md-str" placeholder="STR-XX-000"></div>
                    ...
                    <div class="form-group"><label class="form-label">No. Telepon</label><input class="form-input"
                            name="telepon" id="md-telp" placeholder="+62"></div>
                    <div class="form-group"><label class="form-label">Email</label><input class="form-input"
                            type="email" name="email" id="md-email" placeholder="dokter@glowcare.com"></div>
                    <div class="form-group"><label class="form-label">Pengalaman (Tahun)</label><input
                            class="form-input" type="number" name="pengalaman" id="md-exp" placeholder="0" min="0">
                    </div>
                    <div class="form-group"><label class="form-label">Rating <small
                                style="color:#a89a8a;font-weight:300">(Otomatis dari ulasan
                                pasien)</small></label><input class="form-input" type="text" id="md-rating-display"
                            readonly style="background:#f5f3ee; cursor:default; color:#735a39; font-weight:600;"
                            placeholder="Belum ada ulasan"></div>
                    <div class="form-group full"><label class="form-label">Status</label>
                        <select class="form-select" name="status" id="md-status">
                            <option>Aktif</option>
                            <option>Cuti</option>
                            <option>Tidak Aktif</option>
                        </select>
                    </div>
                    <div class="form-group full"><label class="form-label">Foto Dokter</label><input type="file"
                            class="form-input" name="foto" id="md-foto" accept="image/*"></div>
                    <div class="form-group full"><label class="form-label">Bio Singkat</label><textarea
                            class="form-textarea" name="bio" id="md-bio"
                            placeholder="Deskripsi singkat dokter..."></textarea></div>
                </div>
                <div class="modal-footer">
        ...
        <div class="modal">
            ...
            <form method="POST" id="form-jadwal">
                ...
                <div class="form-row">
                    ...
                    <div class="form-group"><label class="form-label">Jam Mulai</label><input class="form-input"
                            type="time" name="jam_mulai" id="mj-mulai" value="09:00"></div>
                    <div class="form-group"><label class="form-label">Jam Selesai</label><input class="form-input"
                            type="time" name="jam_selesai" id="mj-selesai" value="17:00"></div>
                    <div class="form-group"><label class="form-label">Max Pasien/Hari</label><input class="form-input"
                            type="number" name="max_pasien" id="mj-max" value="8" min="1"></div>
                    ...
                    <div class="form-group full"><label class="form-label">Status</label>
                        <select class="form-select" name="status" id="mj-status">
                            <option>Aktif</option>
    ...
    <div class="modal-overlay" id="modal-treatment" onclick="closeModalOutside(event,'modal-treatment')">
        <div class="modal" style="width:600px">
            ...
            <form method="POST" id="form-treatment" enctype="multipart/form-data">
                ...
                <div class="form-row">
                    <div class="form-group full"><label class="form-label">Nama Treatment</label><input
                            class="form-input" name="nama" id="mt-nama" placeholder="Contoh: Facelift Procedures"
                            required></div>
                    <div class="form-group"><label class="form-label">Kategori</label>
                        <select class="form-select" name="kategori" id="mt-kategori">
                            <option>Acne & Clear Skin</option>
                    ...
                    <div class="form-group"><label class="form-label">Estimasi Durasi</label><input class="form-input"
                            name="durasi" id="mt-durasi" placeholder="Contoh: 60 Menit"></div>
                    <div class="form-group"><label class="form-label">Urutan Tampil</label><input class="form-input"
                            type="number" name="urutan" id="mt-urutan" value="1" min="1"></div>
                    <div class="form-group full">
                        <label class="form-label">Gambar Treatment</label>
                        <div id="mt-gambar-preview-wrap" style="margin-bottom:8px; display:none">
                            <img id="mt-gambar-preview" src="" alt="Preview"
                                style="max-width:100%; max-height:180px; border-radius:8px; border:1px solid #efebe4; object-fit:cover;">
                        </div>
                        <input type="file" class="form-input" name="gambar" id="mt-gambar" accept="image/*"
                            onchange="previewTreatmentImage(this)" style="padding:8px;">
                        <input type="hidden" name="gambar_url_lama" id="mt-gambar-lama">
                        <div style="font-size:11px;color:#a89a8a;margin-top:4px;">Format: JPG, PNG, WEBP. Maks. 2MB.
                            Kosongkan jika tidak ingin mengubah gambar.</div>
                    </div>
                    <div class="form-group full"
                        style="background:#f5f3ee;padding:12px;border-radius:8px;border:1px dashed #d1c4b8">
                        <p style="font-size:12px;color:#7a7571;margin:0">💡 Halaman detail treatment akan otomatis
                            dibuat berdasarkan data di atas. Preview tersedia di tabel setelah disimpan.</p>
                    </div>
                    <div class="form-group full"><label class="form-label">Deskripsi Singkat (tampil di
                            index)</label><textarea class="form-textarea" name="deskripsi" id="mt-desc"
                            placeholder="Deskripsi singkat..."></textarea></div>
                    <div class="form-group full"><label class="form-label">Deskripsi Lengkap</label><textarea
                            class="form-textarea" style="min-height:90px" name="deskripsi_panjang" id="mt-desc-panjang"
                            placeholder="Deskripsi lengkap treatment..."></textarea></div>
                    <div class="form-group full"><label class="form-label">Status</label>
                        <select class="form-select" name="status" id="mt-status">
                            <option>Aktif</option>
                            <option>Nonaktif</option>
                        </select>
                    </div>
            ...
            <form method="POST" action="../../backend/admin/simpan_keuangan.php" id="form-keuangan">
                ...
                <div class="form-row">
                    ...
                    <div class="form-group full">
                        ...
                        <div class="keu-toggle-wrap">
                            <label class="keu-toggle-opt keu-toggle-masuk">
                                <input type="radio" name="jenis" value="Pemasukan" id="keu-r-masuk" required
                                    onchange="updateKeuKategori('Pemasukan')">
                                <span>↑ Pemasukan</span>
                            </label>
                            <label class="keu-toggle-opt keu-toggle-keluar">
                                <input type="radio" name="jenis" value="Pengeluaran" id="keu-r-keluar"
                                    onchange="updateKeuKategori('Pengeluaran')">
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-input" type="number" name="jumlah" id="mkeu-jumlah" placeholder="0" min="1"
                            step="1" required>
                    </div>
                    ...
                    <div class="form-group full">
                        ...
                        <input class="form-input" name="keterangan" id="mkeu-ket"
                            placeholder="Contoh: Pembayaran treatment Botox pasien Siti" required>
                    </div>
                    <div class="form-group">
    ...
    <div class="modal-overlay" id="modal-pesan" onclick="closeModalOutside(event,'modal-pesan')">
        <div class="modal" style="width:580px;max-height:90vh;overflow-y:auto">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:18px">
                ...
                <div class="form-group" style="grid-column:1/-1">
                    ...
                    <div class="form-textarea" id="mp-detail-pesan"
                        style="background:#F9F7F2;cursor:default;min-height:90px;line-height:1.6;white-space:pre-wrap">
                    </div>
                </div>
            </div>
            ...
            <div id="mp-balasan-box" style="display:none;margin-bottom:18px">
                <div
                    style="font-size:10px;letter-spacing:1.5px;text-transform:uppercase;color:#735a39;font-weight:600;margin-bottom:8px">
                    Balasan Admin</div>
                <div style="background:linear-gradient(135deg,#f5f3ee,#F9F7F2);border-left:3px solid #735a39;border-radius:8px;padding:14px 16px;font-size:13px;color:#2D3436;line-height:1.6;white-space:pre-wrap"
                    id="mp-balasan-isi"></div>
            ...
            <div id="mp-form-balas">
                <div
                    style="font-size:10px;letter-spacing:1.5px;text-transform:uppercase;color:#2D3436;font-weight:600;margin-bottom:8px">
                    Tulis Balasan</div>
                <form method="POST" action="../../backend/admin/kelola_pesan.php" id="form-balas-pesan">
                    ...
                    <div style="font-size:11px;color:#94a3b8;margin-bottom:14px">Balasan akan dikirim ke email
                        pengunjung secara otomatis.</div>
                    <div style="display:flex;gap:10px;justify-content:flex-end;align-items:center;">
                        <button type="button" class="btn-cancel" style="padding:7px 16px;font-size:10px"
                            onclick="closeModal('modal-pesan')">Tutup</button>
                        <button type="button" class="btn-save" id="mp-btn-baca"
                            style="background:#8c7d70;display:none;padding:7px 16px;font-size:10px"
                            onclick="tandaiPesanBaca()">Tandai Sudah Dibaca</button>
                        <button type="submit" class="btn-save" id="mp-btn-kirim"
                            style="padding:7px 16px;font-size:10px">Kirim Balasan</button>
                    </div>
                </form>
            </div>
        </div>
            ...
            <form method="POST" action="../../backend/admin/simpan_pengumuman.php" id="form-pengumuman">
                ...
                <div class="form-row">
                    <div class="form-group full">
                        ...
                        <input class="form-input" name="judul" id="mpe-judul"
                            placeholder="Contoh: Jadwal Libur Lebaran Klinik" required>
                    </div>
                    <div class="form-group full">
                    ...
                    <div class="form-group full">
                        ...
                        <textarea class="form-textarea" name="konten" id="mpe-konten" style="min-height:120px;"
                            placeholder="Tulis isi pengumuman secara lengkap..." required></textarea>
                    </div>
                </div>
                <div class="modal-footer">
            ...
            <form method="POST" action="../../backend/admin/proses_bayar.php" id="form-pembayaran">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input class="form-input" id="pay-pasien" readonly
                            style="background:#faf6ee; border-color:#efebe4; cursor:default">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Dokter Pemeriksa</label>
                        <input class="form-input" id="pay-dokter" readonly
                            style="background:#faf6ee; border-color:#efebe4; cursor:default">
                    </div>
                    <div class="form-group full">
                        <label class="form-label">Layanan / Tindakan</label>
                        <input class="form-input" id="pay-treatment" readonly
                            style="background:#faf6ee; border-color:#efebe4; cursor:default">
                    </div>
                    <div class="form-group full">
                        <label class="form-label">Total Biaya Tindakan (Rp)</label>
                        <input type="number" class="form-input" name="jumlah" id="pay-jumlah" min="1"
                            placeholder="Masukkan total nominal yang harus dibayar..." required>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Metode Pembayaran</label>
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-input" name="referensi" id="pay-ref"
                            placeholder="No. ref transfer / slip kasir">
                    </div>
                </div>
                <div class="modal-footer">
        ...
        ['mkeu-kategori', 'mkeu-metode'].forEach(id => {
            ...
            if (el) {
                [...el.options].forEach(o => {
                    if (o.text === data[key] || o.value === data[key]) el.value = o.value;
                });
            }
        });
        openModal('modal-keuangan');
    ...
    <div id="logout-modal"
        style="display:none;position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,0.45);backdrop-filter:blur(4px);align-items:center;justify-content:center">
        <div
            style="background:#fff;border-radius:20px;padding:40px 36px;width:360px;text-align:center;box-shadow:0 24px 64px rgba(0,0,0,0.18);animation:logoutFadeIn .25s ease">
            <div
                style="width:56px;height:56px;background:#fef0f0;border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 20px;font-size:22px">
                ⏻</div>
            <div style="font-family:'Playfair Display',serif;font-size:20px;color:#2D3436;margin-bottom:10px">Yakin
                ingin <em>keluar</em>?</div>
            <p style="font-size:13px;color:#64748b;margin-bottom:28px;line-height:1.6">Sesi Anda sebagai
                <strong>Admin</strong> akan diakhiri. Anda perlu login kembali untuk mengakses dashboard.</p>
            <div style="display:flex;gap:12px;justify-content:center">
                <button onclick="hideLogoutModal()"
                    style="flex:1;padding:11px;border:1.5px solid #d1c4b8;border-radius:50px;background:#fff;color:#64748b;font-size:12px;font-weight:500;letter-spacing:1px;text-transform:uppercase;cursor:pointer;font-family:'DM Sans',sans-serif">Batal</button>
                <a href="../../backend/auth/logout.php"
                    style="flex:1;padding:11px;border-radius:50px;background:#e05050;color:#fff;font-size:12px;font-weight:500;letter-spacing:1px;text-transform:uppercase;cursor:pointer;font-family:'DM Sans',sans-serif;text-decoration:none;display:flex;align-items:center;justify-content:center">Ya,
                    Keluar</a>
            </div>
        </div>
    </div>
